...
class TweetSetSuite {
  ...
  @Test def `most retweeted`: Unit =
    new TestSets {
      val set6 = set5.incl(new Tweet("e", "e", 50))
      assertEquals(50, set6.mostRetweeted.retweets)
    }

  @Test def `apple and google trending`: Unit =
    new TestSets {
      val t0 = System.nanoTime()
      GoogleVsApple.googleTweets.descendingByRetweet
