...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    // Code for other syntactic forms
    case 'def :: (Symbol(name) :: args) :: body_sugar :: expr :: Nil => // GLOBAL or LOCAL
      val body = List('lambda, args, body_sugar)
      if (env == globalEnv)
        globalEnv = env.extendRec(name, env1 => eval(body, env1))
      eval(expr, env.extendRec(name, env1 => eval(body, env1))) // evaluate
    
    case 'case :: expr_scrut :: cases => {
      def reccursiveCheck(c:List[Data]):Data = {
        val currentCase = asList(c.head)
        val value = eval(expr_scrut, env)
    ...
    /*
    case 'case :: expr_scrut :: pairs =>
      val evaluated_expr = eval(expr_scrut, env)
      pairs match {        
        case ('else :: expr_else) => 
          eval(expr_else, env)
        
        case (expr :: value) :: next_pairs =>
          if(evaluated_expr == value)
            eval(expr, env)
          else
            eval('case :: expr_scrut :: next_pairs, env)
        
        case _ => sys.error(s"bad arguments for case statement $pairs")
      }*/
